...
其中，origin与standard目录分别包含原始数据与标准数据，两个目录下分别含有GUID命名的JSON文档，即数据集文件。
...
其中index目录保存病人的档案数据，档案包仅有此目录。此目录下的patient_index.json文件包含患者档案在机构的访问路径和部分平台所需要的摘要数据，其结构如下：

	{
		...
		// 数据集摘要信息，即数据集的关键数据元，此部分数据可扩展，平台直接用于做部分统计。
		summary:
		[
			{
				"数据集标识":
...
鉴于部分医疗机构会自己存储档案，平台对档案的存储管理模式分为两种：集中式（标准档案）与分布式（轻量级档案）。用户通过健康档案浏览器访问时是透明的，不受影响。
