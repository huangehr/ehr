...
public class StdKeySchema extends KeySchema {
    ...
    @Value("${redis-key-schema.std.version-name-column}")
    ...
    public String dictEntryValue(String version, long dictId, String entryCode) {
        assert StringUtils.isNotBlank(version) &&
                dictId > 0 &&
                StringUtils.isNotBlank(entryCode);
