...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        sidePagination: 'client', // client or server 本地或远程分页
        isOnlyPaginationNvi : true,//是否只显示分页导航条，不显示其他信息文本
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        //TODO 翻页逻辑
        if(this.options.isOnlyPaginationNvi){
            html.push(
                '<div class="pull-center pagination">',
                '<ul class="pagination">',
            ...
            if(this.totalPages < 9){
            ...
            else if(this.options.pageNumber<5){
                for(var i=1;i<=5;i++)
                    pageLink.push(i);
                pageLink.push(-1);
                pageLink.push(this.totalPages-1);
                pageLink.push(this.totalPages);
            }
            else if(this.options.pageNumber>this.totalPages-4){
                pageLink.push(1);
                pageLink.push(2);
                pageLink.push(-1);
                ...
                pageLink.push(2);
                pageLink.push(-1);
                pageLink.push(this.options.pageNumber-1);
                pageLink.push(this.options.pageNumber);
                pageLink.push(this.options.pageNumber+1);
                pageLink.push(-1);
                pageLink.push(this.totalPages);
            }
            ...
            for (i = 0; i < pageLink.length; i++) {
                v = pageLink[i];
                if(v==-1)
                    html.push('<li class=" disabled-nb">',
                        '<a href="javascript:void(0)">', "..." ,'</a>',
                        '</li>');
                else
                    html.push('<li class="page-number' + (v === this.options.pageNumber ? ' active disabled' : '') + '">',
    ...
    BootstrapTable.prototype.resetHeader = function () {
        ...
        setTimeout(function () {
            ...
            that.$body.find('tr:first-child:not(.no-records-found) > *').each(function(i) {
                //修改
                that.$header_.find('div.th-inner').eq(i).width($(this).innerWidth()-16);
                var elm = that.$header_.find('div.fht-cell').eq(i);
                elm.width($(this).innerWidth());
                if(i!=0){
                    elm.css('position','absolute');
                    elm.css('top','8px');
                    elm.css('borderLeft','1px solid white');
                    elm.css('height','38px');
                }
            });
