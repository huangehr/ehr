...
(function ($)
{
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _setUrl: function (url)
        {
            ...
            if (p.load)
            {
                // TODO [代码片段001][修正load方式加载dialog内容页面时多加载一次，并且url无携带任何请求参数的问题][yezehua]
                //g.dialog.body.html("").load(p.url, function ()
                //{
                //    g.trigger('loaded');
                //});
                // TODO [代码片段001][修正load方式加载dialog内容页面时多加载一次，并且url无携带任何请求参数的问题][yezehua] 结束
            }
            else if (g.jiframe)
            {
    ...
    $.ligerDialog.waitting = function (title)
    {
        ...
        // TODO [代码片段003][修改等待中类型值为‘waitting’][yezehua]
        //return $.ligerDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
        return $.ligerDialog.open({ cls: 'l-dialog-waittingdialog', type: 'waitting', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
        // TODO [代码片段003][修改等待中类型值为‘waitting’][yezehua] 结束
    };
