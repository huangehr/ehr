...
import com.yihu.ehr.adaption.dao.OrgDictItemRepository;
...
@Service
public class OrgDictItemService extends BaseJpaService<OrgDictItem, OrgDictItemRepository> {


    @Transactional(propagation = Propagation.SUPPORTS)
    public boolean isExistOrgDictItem(long orgDictSeq, String orgCode, String code) {
        return ((OrgDictItemRepository) getRepository()).isExistOrgDictItem(orgDictSeq, orgCode, code).size() != 0;
    }
    ...
    public OrgDictItem getOrgDicEntryBySequence(String orgCode,int sequence){
        return ((OrgDictItemRepository) getRepository()).getOrgDicEntryBySequence(orgCode, sequence);
    }
}
