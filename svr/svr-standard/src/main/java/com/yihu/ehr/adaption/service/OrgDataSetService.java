...
import com.yihu.ehr.adaption.dao.OrgDataSetRepository;
...
@Transactional
@Service
public class OrgDataSetService extends BaseJpaService<OrgDataSet, OrgDataSetRepository> {
    @Autowired
    OrgMetaDataService orgMetaDataService;
    @Autowired
    private OrgDataSetRepository orgDataSetRepository;
    ...
    public boolean isExistOrgDataSet(String orgCode, String code) {
        ...
        return ((OrgDataSetRepository) getRepository()).isExistOrgDataSet(orgCode, code).size() != 0;
    }
    ...
    {
        return ((OrgDataSetRepository) getRepository()).getDataSetBySequence(orgCode,sequence);
    }
}
