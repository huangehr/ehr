...
public class GeographyDictService extends BaseJpaService<GeographyDict,XGeographyDictRepository>{
    ...
    public GeographyDict findById(String id) {
        return geographyDictRepository.findOne(Long.valueOf(id));
    }

    public GeographyDict findByName(String name) {
        List<GeographyDict> DictList =geographyDictRepository.findByName(name);
        GeographyDict geographyDict=new GeographyDict();
        if(null!=DictList){
