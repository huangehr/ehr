...
public class ResourcesQueryService  {
    ...
    public Envelop getResources(String resourcesCode,String appId,String orgCode,String queryParams,Integer page,Integer size) throws Exception {
        ...
        if(rsResources != null) {
            ...
            //获取结果集
            envelop = getResultData(resourcesCode, queryParams,  page, size, grantType, methodName, metadataList, true);
        }
        else{
    ...
    public Envelop resourcesBrowse(String resourcesCode, String appId, String orgCode, String queryParams, Integer page, Integer size) throws Exception {
        Envelop envelop = null;
        //获取资源信息
        RsResources rsResourcess = resourcesDao.findByCode(resourcesCode);
        if(rsResourcess != null) {
            String methodName = rsResourcess.getRsInterface(); //执行函数
            String grantType = rsResourcess.getGrantType(); //访问方式 0开放 1授权
            //资源结构
            List<DtoResourceMetadata> metadataList = isAccessMetadata(resourcesCode, grantType, appId);
            //获取Saas权限参数
            String saas = getSaas(appId, orgCode);
            ...
            //获取结果集
            envelop = getResultData(resourcesCode, queryParams,  page, size, grantType, methodName, metadataList, false);
            //如果资源查询为详细信息则增加主表信息
            if(methodName.endsWith("Sub") && envelop.isSuccessFlg()) {
    ...
    private Envelop getResultData(String resourcesCode, String queryParams, Integer page, Integer size, String grantType, String methodName, List<DtoResourceMetadata> metadataList, boolean isSpecialScan) throws Exception{
