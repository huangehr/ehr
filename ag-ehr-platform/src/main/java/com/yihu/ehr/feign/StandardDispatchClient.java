...
public interface StandardDispatchClient {
    ...
    @RequestMapping(value = ApiVersion.Version1_0+ ServiceApi.Standards.Dispatches, method = RequestMethod.GET)
    @ApiOperation(value = "获取适配方案摘要", response = RestEcho.class, produces = MediaType.APPLICATION_JSON_UTF8_VALUE, notes = "获取两个指定版本的标准化数据差异与适配方案，文件以Base64编码，压缩格式为zip")
